(ns gtinalloc.actor
  "GTINAllocationActor — the GS1 Prefix & GTIN Allocation actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one allocation request
  ...
  The unconditional invariant: the Allocation Advisor can never
  directly issue a GTIN the GTINAllocationGovernor refuses — every
  issue! call is gated behind `:decide`."
...
(defn build-graph
  "Build a compiled GTINAllocationActor graph. `store` implements
  `gtinalloc.store/Store`. `advisor` implements
  `gtinalloc.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn run-request!
  "Run one allocation request to completion or interrupt. `thread-id`
